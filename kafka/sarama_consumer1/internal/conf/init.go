...
func parseConfigFromPath(configPath string) error {
	...
	if err != nil {
		return fmt.Errorf("[CONFIG] error reading config file, %s", err)
	}
	if err := yaml.Unmarshal(bytes, conf); err != nil {
		return fmt.Errorf("[CONFIG] unable to decode into struct, %v", err)
	}
	fmt.Printf("[CONFIG] success read and parse app config from path: %v\n", configPath)
	return nil
}
